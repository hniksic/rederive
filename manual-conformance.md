...
**238 pass, 156 do not**, in about 17 seconds.

The 156 are marked `xfail` through `NOT_YET_HELD`, a manifest of test ids at the top of
...
wedging the suite. They are counted among the 157.
...
- `-1/COS(x)^2` for `-TAN(x)^2 - 1`; `LN(256)` for `8*LN(2)`; `1 - 1/2^m` for `1 - 2^(-m)`.
