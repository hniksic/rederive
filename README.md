...
- `src/rederive/model/` - pure Python, no Textual: the expression tree, the
  system control settings, and the session, which holds the history, the parse
  state, each entry's render, and all navigation rules.
- `src/rederive/syntax/` - Derive expression text to expression trees. It does
  no mathematics: `2+3` is a sum node, never `5`.
